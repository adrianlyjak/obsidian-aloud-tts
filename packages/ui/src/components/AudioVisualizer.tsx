...
import { DecodedAudioData } from "open-tts";
...
export const AudioVisualizer: React.FC<AudioVisualizerProps> = ({
  ...
  const fft = React.useMemo(() => new FFT(512), [decodedAudio]);
  React.useEffect(() => {
    if (ref.current && audioElement && decodedAudio) {
  ...
  }, [ref.current, audioElement, decodedAudio, timelineStartSeconds, fft]);
...
function getFrequencyBins(
  ...
  const position =
    (audioElement.currentTime - timelineStartSeconds) * decodedAudio.sampleRate;
  const mono = decodedAudio
    .getChannelData(0)
