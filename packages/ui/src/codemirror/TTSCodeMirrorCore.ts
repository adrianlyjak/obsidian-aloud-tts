...
import { AudioTextChunk } from "open-tts";
import { TextEdit } from "open-tts";
...
function createTTSHighlightField(
  ...
  player: AudioStore,
  onAutoscroll?: (range: TTSAutoscrollRange) => void,
) {
...
export function createTextChangeHandler(
  ...
  return EditorView.updateListener.of((update: ViewUpdate) => {
    ...
    if (update.viewportChanged && bridge.activeEditor === update.view) {
      ...
      const lastProgrammaticScroll = player.getLastProgrammaticScroll();
