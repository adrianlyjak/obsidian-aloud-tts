...
import { AudioSink, DecodedAudioData } from "./AudioSink";
...
import { FakeAudioSink, createTestModel } from "../test-utils";
...
describe("AudioStore", () => {
  ...
  describe("Active Track", () => {
        ...
        },
        {
          ...
          audioSink: new FakeAudioSink({
            decodeAudioData: async (ab: ArrayBuffer) =>
              ({ duration: 5 }) as DecodedAudioData,
          }),
        },
      );
    ...
    test("track switching should remain timely when playback rate changes", async () => {
      ...
      const duration = 5;
      const sink = new FakeAudioSink({
        decodeAudioData: async () => ({ duration }) as DecodedAudioData,
      });
      const settings = mobx.observable({ ...DEFAULT_SETTINGS });
