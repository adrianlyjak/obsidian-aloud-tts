...
import { AudioTextContext } from "../models/tts-model";
...
export class ChunkPlayer {
    ...
    while (!this.isDestroyed) {
      ...
      if (transitionType === "position-changed") {
        ...
        if (start <= position && position <= end) {
          ...
          await this.system.audioSink.waitForSeeking();
        } else {
          toReset = { all: true };
        ...
        const bufferedStart = this.system.audioSink.bufferedStart;
        if (
          seekedAtTime != null &&
          bufferedStart != null &&
  ...
  _whenSeeked(): CancellablePromise<"seeked"> {
    const baseline = this.system.audioSink.currentTime;
    return this.system.audioSink.waitForSeeking().thenCancellable(() => {
      const newTime = this.system.audioSink.currentTime;
...
export function getPositionAccordingToPlayback(
  ...
  const audioPosition = audioSink.currentTime;
...
function pruneEvictedChunkState(
  ...
  const bufferedStart = audioSink.bufferedStart;
  ...
  for (const chunk of activeText.audio.chunks) {
    ...
    if (chunkEnd <= bufferedStart) {
      // Playback storage has already evicted this media range. Drop chunk
      // bytes/state so it can be reloaded on demand, but preserve timing
      // metadata for stable seek mapping.
...
function releaseDecodedAudioBehindPosition(activeText: ActiveAudioText): void {
  ...
  // - This function releases decoded PCM kept on chunk objects.
  //
  ...
  for (let i = 0; i < currentPos - 1; i++) {
    ...
    if (oldChunk?.decodedAudio) {
      oldChunk.releaseDecodedAudio();
    }
  }
}
    ...
    return loadCheck(
      ...
      if (maybe && activeText && position !== null && !cancelled) {
        ...
        return system.audioSink
          ...
          .then(() => system.audioSink.decodeAudioData(result.data))
          .then((buff) => {
            const chunk = activeText.audio.chunks[position];
            if (chunk.audio) {
