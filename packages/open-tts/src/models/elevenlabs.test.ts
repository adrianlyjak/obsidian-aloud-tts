...
describe("ElevenLabs Model", () => {
  ...
  describe("elevenLabsCallTextToSpeech", () => {
    ...
    it("should make correct API call for basic TTS request", async () => {
      const mockAudioBytes = new ArrayBuffer(1024);
      const mockResponse = {
        ok: true,
        status: 200,
        arrayBuffer: vi.fn().mockResolvedValue(mockAudioBytes),
      };
      ...
      expect(result.data).toBe(mockAudioBytes);
      expect(result.format).toBe("mp3");
    });

    it("should include voice settings when provided", async () => {
      const mockAudioBytes = new ArrayBuffer(512);
      const mockResponse = {
        ok: true,
        status: 200,
        arrayBuffer: vi.fn().mockResolvedValue(mockAudioBytes),
    ...
    it("should include context when available", async () => {
      const mockAudioBytes = new ArrayBuffer(256);
      const mockResponse = {
        ok: true,
        status: 200,
        arrayBuffer: vi.fn().mockResolvedValue(mockAudioBytes),
