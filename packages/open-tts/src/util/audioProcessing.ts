...
import { DecodedAudioData } from "../player/AudioSink";
...
export async function concatenateMp3Buffers(
  ...
  const decodedAudios: DecodedAudioData[] = [];
  for (const buffer of buffers) {
    const decodedAudio = await audioSink.decodeAudioData(buffer);
    decodedAudios.push(decodedAudio);
  }

  // Get audio properties from first buffer
  const sampleRate = decodedAudios[0].sampleRate;
  const numberOfChannels = decodedAudios[0].numberOfChannels;

  // Calculate total length and concatenate PCM data
  const totalLength = decodedAudios.reduce((sum, buf) => sum + buf.length, 0);
