...
describe("audioProcessing", () => {
  ...
  describe("concatenateMp3Buffers", () => {
    ...
    const createMockAudioSink = (audioBuffers: DecodedAudioData[]) => {
      let callCount = 0;
      return {
    ...
    it("should concatenate multiple audio buffers correctly", async () => {
      ...
      const buffer1 = createMockDecodedAudioData(1000);
      const buffer2 = createMockDecodedAudioData(500);
      const buffer3 = createMockDecodedAudioData(750);
    ...
    it("should preserve audio properties from first buffer", async () => {
      ...
      const buffer1 = createMockDecodedAudioData(1000, 48000, 1); // 48kHz, mono
      const buffer2 = createMockDecodedAudioData(500, 44100, 2); // 44.1kHz, stereo (should be ignored)
