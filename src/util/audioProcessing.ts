...
export async function pcmBufferToMp3Buffer(
  ...
  return new Promise((resolve, reject) => {
    try {
      let pcmSamples: Int16Array; // lamejs.Mp3Encoder expects Int16Array
      ...
      if (options.bitDepth !== 16) {
        ...
        reject(new Error(`Raw PCM bitDepth ${options.bitDepth} is not directly supported. Please provide 16-bit PCM.`));
        return;
      }
      if (audioArrayBuffer.byteLength % (options.channels * (options.bitDepth / 8)) !== 0 && options.channels > 0) {
        console.warn("Raw PCM ArrayBuffer length is not a perfect multiple of (channels * bytesPerSample). Data might be truncated or padded by ArrayBuffer views if not aligned.");
      }
      if (audioArrayBuffer.byteLength % 2 !== 0 && options.bitDepth === 16) {
        reject(new Error("Raw 16-bit PCM ArrayBuffer length must be even to create Int16Array."));
        return;
      }

      if (options.channels > 2 || options.channels < 1) {
        reject(new Error(`Unsupported number of channels: ${options.channels}. Only 1 or 2 channels are supported.`));
        return;
      }

      // The entire buffer is PCM data
      pcmSamples = new Int16Array(audioArrayBuffer);
      ...
      const mp3Encoder = new lamejs.Mp3Encoder(options.channels, options.sampleRate, kbpsToUse);
      const mp3Data: Uint8Array[] = [];
      const sampleBlockSize = 1152; // Standard block size for MP3 encoder

      for (let i = 0; i < pcmSamples.length; i += sampleBlockSize * options.channels) {
      ...
      if (mp3Data.length === 0) {
        reject(new Error("MP3 encoding resulted in no data. Check PCM input and encoder parameters."));
        return;
      }
